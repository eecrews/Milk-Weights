...
public class MilkOperations {
	...
	public class MilkData implements Comparable<MilkData> {
		...
		public int compareTo(MilkData k) {
			Integer kID = Integer
					.parseInt(k.getF().getID().substring(4).trim());
			Integer FID = Integer
					.parseInt(this.getF().getID().substring(4).trim());
	...
	public ArrayList<MilkData> farmReport(String farmID, int year) {
		...
		for (int i = 0; i < Farms[ind].getEntryList().size(); i++) {
			if (Farms[ind].getEntryList().getMilkDate(i) != null && Farms[ind]
					.getEntryList().getMilkDate(i).getYear() == year) {
				weight += Farms[ind].getEntryList().getMilkWeight(i);
				weights.get(
						Farms[ind].getEntryList().getMilkDate(i).getMonthValue()
								- 1)
						.addToAmount((int) Farms[ind].getEntryList()
								.getMilkWeight(i));// Adding amount to specific
													// month
			}
		}
		...
		for (int i = 0; i < weights.size() - 1; i++) {
			weights.get(i)
					.setPercentage((weights.get(i).getAmount() / weight) * 100);
		}
	...
	/**
	 * Returns list of farms in the given year along with the output and
	 * percentages of said farms for that year.
	 * 
	 * @param year
	 * @return
	 */
	public ArrayList<MilkData> AnnualReport(int year) {
		...
		for (int i = 0; i < Farms.length; i++) {
			for (int j = 0; j < Farms[i].entryList.size(); j++) {
				if (Farms[i].getEntryList().getMilkDate(j) != null && Farms[i]
						.getEntryList().getMilkDate(j).getYear() == year) {
		...
		for (int i = 0; i < farms.size(); i++) {
			...
			weights.add(new MilkData(farms.get(i),
	...
	/**
	 * Returns list of farms for given month as well as output and percentage
	 * for each farm.
	 * 
	 * @param year
	 * @param month
	...
	public ArrayList<MilkData> MonthlyReport(int year, int month) {
		...
		for (int i = 0; i < Farms.length; i++) {
			for (int j = 0; j < Farms[i].entryList.size(); j++) {
				...
				if (Farms[i].getEntryList().getMilkDate(j) != null
						&& Farms[i].getEntryList().getMilkDate(j)
								.getYear() == year
						&& Farms[i].getEntryList().getMilkDate(j)
								.getMonthValue() == month) {
		...
		for (int i = 0; i < farms.size(); i++) {
			double total = farmReport(farms.get(i).getID(), year).get(month - 1)
					.getAmount();
			double percent = total * 100 / sold;
			System.out.println(farms.get(i).getID() + ": "
					+ farmReport(farms.get(i).getID(), year).get(month - 1)
							.getAmount());
			weights.add(new MilkData(farms.get(i),
					farmReport(farms.get(i).getID(), year).get(month - 1)
							.getAmount(),
					percent));// should be farm, total output of farm for that
	...
	public ArrayList<MilkData> DateRangeReport(int year1, int month1, int day1,
			int month2, int day2) {
		LocalDate D1 = LocalDate.of((Integer) year1, (Integer) month1,
				(Integer) day1);
		LocalDate D2 = LocalDate.of((Integer) year1, (Integer) month2,
				(Integer) day2);
		ArrayList<MilkData> weights = new ArrayList<MilkData>();
		ArrayList<Farm> farms = new ArrayList<Farm>();
		int sold = 0;
		for (int i = 0; i < Farms.length; i++) {
			for (int j = 0; j < Farms[i].entryList.size(); j++) {
				if (Farms[i].getEntryList().getMilkDate(j) != null) {
					...
					if (D3.getDayOfYear() >= D1.getDayOfYear()
							&& D3.getDayOfYear() <= D2.getDayOfYear()) {// D3 is
																		// between
																		// D1
																		// and
																		// D2
						...
						if (!farms.contains(Farms[i])) {// Farm has not already
														// been added
							farms.add(Farms[i]);
							weights.add(new MilkData(Farms[i],
									(int) Farms[i].getEntryList()
											.getMilkWeight(j),
									Farms[i].getEntryList().getMilkWeight(j)
											/ sold));
